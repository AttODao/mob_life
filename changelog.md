...
- Reworked normal Mob form movement. W moves forward, A and D turn the body and
  view, backward input is ignored, and movement now retains natural friction,
  drag, collision, and outside forces while better matching each selected mob.
- Reworked jumping for each kind of Mob form. Ordinary forms use their natural
  jump strength, Rabbits automatically hop with their native rhythm, and Horse,
  Donkey, and Mule forms use a charged jump while sprinting.
- Smoothed body, head, and camera turning in both normal and Instinct modes.
  The view now returns naturally toward the body's direction without fighting
  attacks, interactions, or direct camera input.
- Improved Instinct movement by preventing duplicate motion, restoring natural
  exits from water, and avoiding sudden or incorrect turns across angle
  boundaries.
- Restored movement view bob while travelling in Instinct mode. Rabbit forms
  keep their steady first-person view in both normal and Instinct modes.
- Replaced the blocky brown Instinct screen border with a smooth rounded effect.
