...
public record MorphDefinition(MorphType type, CompoundTag nbt) {
    public static final Codec<MorphDefinition> CODEC =
        RecordCodecBuilder.create(instance ->
            instance
                .group(
                    MorphType.CODEC.fieldOf("morph").forGetter(
                        MorphDefinition::type
                    ),
                    CompoundTag.CODEC.optionalFieldOf(
                        "nbt",
                        new CompoundTag()
    ...
    private static final List<String> ENTITY_IDENTITY_TAGS = List.of(
        "id",
        "UUID",
        "Pos",
        "Motion",
    ...
    public MorphDefinition {
        ...
        nbt = type.isPlayer() ? new CompoundTag() : sanitize(nbt);
    }
    ...
    private static CompoundTag sanitize(CompoundTag source) {
        CompoundTag sanitized =
            source != null ? source.copy() : new CompoundTag();
        if (!sanitized.contains("variant") && sanitized.contains("Variant")) {
