...
public final class FirstPersonMorphHandRenderer {
	...
	private static ModelPart getHandPart(EntityModelSet currentModelSet, MorphType morph, HumanoidArm arm) {
		if (modelSet != currentModelSet) {
			modelSet = currentModelSet;
			HAND_PARTS.clear();
		}

		ModelPart[] parts = HAND_PARTS.computeIfAbsent(morph, type -> createParts(currentModelSet, type));
	...
	private static ModelPart[] createParts(EntityModelSet currentModelSet, MorphType morph) {
		Model<?> model = switch (morph) {
			case PLAYER -> throw new IllegalArgumentException("Player mode does not have a mob hand model");
			case COW -> new CowModel(currentModelSet.bakeLayer(ModelLayers.COW));
			case SHEEP -> new SheepModel(currentModelSet.bakeLayer(ModelLayers.SHEEP));
			case CHICKEN -> new AdultChickenModel(currentModelSet.bakeLayer(ModelLayers.CHICKEN));
		};
		...
		ModelPart right = model.root().getChild(rightPart);
		ModelPart left = model.root().getChild(leftPart);
